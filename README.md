...
> **当前版本**：v1.2.1 — 托盘「润色开关」、轻清洗模式（去口水字 + 秒出）、浮窗打磨与稳定性修复。
...
- **托盘「润色」勾选开关（v1.2.1）**：双击运行后在系统托盘右键菜单里可随时切换「润色」开/关（状态持久化到 `config.json`）：
  - **开启（默认）**：走原有 LLM 大改写，通顺但需等待（约 1.x–5s）。
  - **关闭（轻清洗）**：纯本地规则、毫秒级「秒出」，不调大模型。只剥短句/纯数字的句末标点，并清理少量低风险口语词（呃/嗯/哦/额/唉）；保留「这个/那个/然后/其实/就是/的话」等本义词，以及「啊/呀/哎/吧」等语气词，避免误伤语义。
