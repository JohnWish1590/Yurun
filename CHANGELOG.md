...
方案讨论、试验、被否决的路线和正式版反复过程另见 [`docs/开发过程-思考尝试与反复.md`](docs/开发过程-思考尝试与反复.md)。本文件保留每个历史版本的可核对变更，不因整理开发文档而删除。

---

## [1.4.4] — 2026-09-21 · 恢复纠错选区读取与明确快捷方式图标

> **发布范围**：恢复 Cindy 等应用中原本可用的自动复制选中文字，同时保持剪贴板堆损坏防护；安装器显式绑定语润图标。

### 改动点

- 将安全剪贴板格式扩展到 `CF_DIB` 和 `CF_DIBV5`，跳过仍不安全的 `CF_BITMAP` 与私有格式；当前只有图片格式的剪贴板不再阻断后续高权限 Ctrl+C 读取。
- 安装器开始为开始菜单和桌面快捷方式显式设置 `语润.exe` 图标，避免快捷方式沿用旧缓存图标。

### 发布文件

- `dist/语润-Setup-1.4.4.exe`

## [1.4.3] — 2026-09-21 · 修复纠错窗口闪退与升级关闭问题

> **发布范围**：修复纠错窗口打开时的原生剪贴板堆损坏风险，并修复升级时旧版主程序未自动关闭的问题。

### 改动点

- 剪贴板事务只处理明确的 HGLOBAL 文本格式，跳过位图、增强图元和应用私有格式；没有安全文本快照时取消自动读取，避免原生句柄误处理导致 `0xc0000374` 堆损坏。
- 安装器升级前同时结束旧版 `语润.exe` 与 `YurunInputHelper.exe`，无需用户手动退出旧版本。

### 发布文件

- `dist/语润-Setup-1.4.3.exe`

## [1.4.2] — 2026-09-21 · 稳定性、输入可靠性与打包修复

> **发布范围**：修复 P0/P1 级输入、助手连接、剪贴板保护、持久化与打包问题；安装方式保持不变。

### 改动点

- 长文本输入改为受控批量发送，并正确处理 UTF-16 代理项，减少高权限窗口中的丢字和卡顿。
- 高权限输入助手增加断线检测、有限重连和本地热键回退，避免助手异常退出后主程序失去响应。
- 纠错剪贴板改为原生格式快照与序列保护，无法安全备份时取消操作，避免覆盖用户剪贴板。
- 配置和个人词典保存改为原子替换；保存失败时不发布错误的内存状态。
- 单实例识别增加进程身份校验，降低误杀其他版本语润进程的风险。
- PyInstaller 显式打包 UI Automation DLL；安装器继续使用管理员模式，并保留高权限输入助手安装流程。

### 发布文件

- `dist/语润-Setup-1.4.2.exe`
...
> **涉及文件**：src/config.py、src/gui.py、src/hotkey.py、src/main.py、src/pill.py、src/sauc_asr.py、src/typer.py、src/privileged_ipc.py、src/privileged_helper.py、src/logger.py、installer/yurun_setup.iss、README.md、RELEASE.md、CHANGELOG.md

### 改动点

- **录音热键可完全自定义**：热键的「键」和「修饰键」分开配置（`hotkey` / `hotkey_modifiers`），修饰键是 Alt / Ctrl / Shift / Win 的任意组合位掩码，裸键仍然允许。键名与修饰键分开存，解析时不需要拆字符串。
- **纠错热键独立且可自定义**：默认 **Alt + `**（沿用手感，左右 Alt 不区分）。纠错热键同样支持修饰键组合，并单独保存在 `correction_hotkey` / `correction_hotkey_modifiers`。
- **热键注册改用 `RegisterHotKey`**：不再依赖低层键盘钩子。非提权的低层钩子（`WH_KEYBOARD_LL` / pynput）在**提权窗口获得焦点时收不到按键**，之前的"在普通软件里能用、切到 Cindy 就没反应"就来自这里。
- **设置界面新增录制控件**：一个录制控件 + 标题右边的「录音 / 纠错」选择框，选哪个就录哪个；录制时临时停用正在生效的两个热键，所以按当前热键不会误触发录音。另附反引号 / Caps Lock / F8 常用预设。
- **保存前真实验证**：修改会立即验证能否注册，无法使用的组合不会覆盖当前可用设置；保存时先把录制控件里的值回存到对应槽位，切换槽位不会丢掉刚录的组合。
- **选中文字即可纠错**：选中一段文字后按纠错热键，弹出「错误纠正」窗口，窗口会先定位再显示，并在填入识别文本后重新计算高度，长文本不会把底部按钮挤出可视区。确认后可直接替换原选区。
- **纠错在提权软件里也能用**：非提权进程的 `SendInput` 会被 UIPI 拦下，因此由高权限输入助手代为把目标窗口置前并发一次 Ctrl+C 读取选区。目标窗口没有成功置前时**不会**发送按键。
- **剪贴板事务化**：纠错窗口使用单例 + 代际号管理剪贴板，确认、取消、重建与异常路径都会恢复原内容，并用剪贴板序号确认 Ctrl+C 真的生效。
- **多显示器与任务栏感知**：纠错窗口按目标窗口所在显示器的工作区做边界收敛，不再拿整个虚拟桌面（含负坐标显示器）计算。
- **重试期间状态可见**：最终结果连接中断后，浮窗会显示「正在重试」；识别侧允许用已缓存的完整音频重试一次，而不是把不完整结果当完整句子输入。
- **设置窗口内容压进屏幕**：设置项总高度会超过屏幕高度上限，之前底部「个人记忆」与「保存并应用 / 取消」会被裁掉且触达不了。现在通过精简与合并说明文字、微调卡片间距把内容压到屏幕内，滚动条不再出现，底部按钮始终可见。
- **子窗口图标修复**：tkinter 的 `Toplevel` 不继承父窗口图标，「管理词库」等子窗口此前显示系统默认图标；现在统一为语润图标。

### 验证

- `python Pre\run_pre_tests.py` **11/11 全部通过**，含设置窗口「录音 / 纠错」选择框 49 项、设置窗口可触达性 10 项、窗口图标 16 项、热键录制与双监听分发、助手热键协议、提权助手冻结产物校验等。
- 真机验证了提权软件（Cindy）中的纠错热键唤出、选区读取与纠错窗口可输入；普通软件路径行为不变。
- 提升到正式版后重新执行了源码语法编译检查与启动冒烟（启动日志输出 `语润 v1.4.0`），并重新构建三个可执行文件与安装包。

### 行为变化

- 纠错热键由 `Ctrl + \`` 改为默认 **`Alt + \``**（Cindy 侧边栏占用了 Ctrl + `）；两个热键都可以在设置里改成任何键或组合键。
- 旧配置没有 `hotkey_modifiers` / `correction_hotkey_modifiers` 字段时按默认补齐（录音 = 裸反引号，纠错 = Alt + 反引号），**升级后手感与原版本一致**，不会擅自改变既有习惯。
- 设置窗口不再出现滚动条；由于设置内容本身已超出屏幕高度上限，个别说明文字做了精简与合并（「必填」并入字段名、「个人记忆」的说明并入标题）。
- 纠错窗口可以记住上次拖动后的位置；显示器布局变化导致位置不可见时会回到目标窗口附近。
- 本版仍是「全局语音输入层」，不是输入法；未注册 TSF 服务，也不修改微软拼音、搜狗等现有输入法的预编辑与候选栏。

---

## [1.3.4] — 2026-09-04 · 浮窗定位与输入动画收尾

> **发布范围**：从 Preview 验收后并入的两项浮窗体验修复：Electron / Chromium 类软件的定位回退，以及高权限长文本输入时的动画节奏。未改变录音、ASR、会话、词库学习、输入法或焦点策略。
> **涉及文件**：src/main.py、src/pill.py、src/logger.py、installer/yurun_setup.iss、README.md、RELEASE.md、CHANGELOG.md

### 改动点

- **浮窗定位更贴近实际输入区域**：当 Windows 无法提供文字插入点、也无法从 UI Automation 识别输入控件时，语润会记录热键按下瞬间的鼠标位置作为本轮稳定锚点，再最后才回退到整个目标窗口。它不会持续追踪鼠标，不会抢焦点，也不会影响输入目标。
- **修复全屏 Electron 顶部误定位**：WorkBuddy、Codex 等全屏或近全屏界面此前可能因只能拿到整个窗口矩形，而把浮窗放在屏幕顶部。现在会优先使用本轮冻结的位置，避免这一回退误差。
- **动画以真实输入进度为准**：浮窗不再用独立倒计时猜测文字进入速度，而是在每个字符被确认送入目标软件后同步缩短。对于高权限输入助手的逐字通信，长文本不会再出现“输入还在继续、浮窗已整段跳没”的不规则现象。
- **保留轻微领先感**：动画会随着进度逐渐取得很小的领先量（短句约 1 字，长句最多 10 字），因此通常会在最后少量文字前完成收尾，但不会明显早于真实输入。
- **纠错框边界明确**：纠错框保持原有 17px 输入字号；拼音预编辑和候选栏属于 Windows 输入法，不由语润窗口强行重绘或修改。

### 验证

- Preview 的独立配置、日志和高权限助手均能启动；主程序已连接 PRE 专用助手。
- 通过 Python 语法编译及动画进度公式检查：100 字预览按 1→11→55→97→99→100 的渐进节奏收尾，而不是在开始阶段清空。
- 正式版构建前将再次执行源码语法检查、打包产物检查和安装器构建。

### 行为变化

- 在普通可访问控件中，精确 caret 与 UIA 输入控件边界仍然优先；冻结鼠标坐标只在前两者不可用时作为安全回退。
- 不新增常驻鼠标监听、TSF 输入法服务或自动焦点切换；日常输入路径和权限模型保持不变。

---

## [1.3.3] — 2026-09-03 · 个人记忆学习与高权限输入兼容

> **发布范围**：从 Preview 验收后并入的两项功能：用户明确确认的个人词库学习，以及对 WorkBuddy 等高权限桌面软件的后台输入兼容。暂停中的 TSF 输入法实验、常驻麦克风与 pre-roll 均未纳入本版。
> **涉及文件**：src/config.py、src/dictionary.py、src/gui.py、src/hotkey.py、src/logger.py、src/main.py、src/tray.py、src/voice_session.py、src/privileged_ipc.py、src/privileged_helper.py、src/input_helper_setup.py、Yurun.spec、installer/yurun_setup.iss、README.md、RELEASE.md、CHANGELOG.md

### 改动点

- **个人记忆学习**：仅在用户主动使用「替换并存入词库」或在管理窗口手动新增时，记录“错词 → 正确词”。同一规则重复确认会累积次数，用于排序和稳定性判断；不会因用户在其他软件中输入了某个词而自动学习。
- **当场替换**：纠错确认后，当前选中的错词会立刻替换为正确词，同时保存个人记忆；无需用户再手动删除、重新输入。
- **词库管理**：托盘和设置页均提供「管理个人记忆」入口，可搜索、编辑、删除、停用与清空本地条目。词库保存于 `%APPDATA%\\Yurun\\user_dictionary.json`，不上传、不监听其他应用键盘、不把剪贴板当学习来源。
- **实际使用路径**：已确认条目可参与 ASR 热词、快速输入的低风险本地纠正和智能整理的词典提示，优先改善姓名、公司名、项目名、缩写与专业术语。
- **高权限软件兼容**：安装版新增没有界面、登录后运行的“语润输入助手”。普通权限的语润主程序经本机认证连接助手；助手只监听配置的语润热键，并只在热键按下时锁定且仍位于前台的目标窗口中输入最终文本。
- **WorkBuddy 修复**：解决 WorkBuddy 更新后以管理员权限运行时，普通语润热键被应用当作反引号字符、或无法把最终文本写入的问题。
- **安全回退**：助手不可用时，语润仍按原有单进程路径服务普通软件；用户在录音期间主动切走窗口时，助手取消本次输入，绝不把文字写进新窗口，也不抢回焦点。
- **安装与卸载**：安装器一次性创建高权限助手任务，日常使用不需要以管理员身份启动语润；卸载时一并删除该任务和本地数据。

### 验证

- Preview 中完成了个人记忆管理、即时替换与本地持久化的自检。
- Preview 真机验证了普通 `run_pre.bat` 连接高权限助手后，WorkBuddy 中的全局热键可正常录音并输入；不再需要每次以管理员身份启动主程序。
- 对助手通信实施了回环地址限制、随机密钥挑战认证和目标前台检查；连接失败时回退而非中断常规输入。
- Python 语法编译、个人记忆自检、助手通信模拟测试和正式打包检查均作为发布前验证项执行。

### 行为变化

- 纠错不再只是一次性替换：你可以选择把确认过的词真正存成自己可管理的长期记忆。
- 使用安装版后，WorkBuddy 等高权限软件也能使用同一套热键；普通日常启动无需管理员权限。
- 本版仍是“全局语音输入层”，不是输入法；未注册 TSF 服务，也不会改变微软拼音、搜狗等现有输入法。

---

## [1.3.2] — 2026-08-30 · 输入可靠性与浮窗定位

> **发布范围**：从 Preview 验收并入的可靠性和浮窗体验改进；不改变快速输入 / 智能整理的产品路径，不引入焦点策略实验。
> **涉及文件**：src/main.py、src/voice_session.py、src/sauc_asr.py、src/pill.py、src/gui.py、src/logger.py、Yurun.spec、requirements.txt、installer/yurun_setup.iss、README.md、RELEASE.md、CHANGELOG.md

### 改动点

- **会话隔离**：每次按住说话拥有独立的目标窗口、停止信号、配置快照和识别时间线；前一句尚在输入时开始下一句，不会混字或误投到新窗口。
- **完整结果保护**：SAUC 只在收到服务端最终结果后输入；网络在最终结果前关闭会明确报识别失败，而非把不完整的中间结果当成完整句子输入。
- **输入失败可见**：逐字输入无法确认时会停止该句并提示「输入失败」，不静默吞字，也不危险地自动重试造成重复。
- **会话轨迹**：新增 `%APPDATA%\\Yurun\\logs\\session-trace.jsonl`，仅记录按下、松开、最终识别、入队和完成/失败等脱敏事件，不保存语音正文。
- **浮窗定位**：优先使用 UI Automation 的真实插入点、输入控件边界和记录的目标窗口；过滤 Chromium/Electron 偶发的页面顶部假光标。多显示器和不同任务栏位置按对应屏幕工作区定位。
- **浮窗反馈**：完整输入后短暂显示「输入完成」；识别阶段同样显示连续转圈。

### 验证

- Python 编译检查通过；会话轨迹、Final-only 识别、逐字输入失败保护和多显示器定位模拟测试通过。
- 当前 Stable 保持独立可运行；后续变更另从本版复制到新的 `Pre/`，不直接修改正式代码。

---

## [1.3.1] — 2026-08-30 · 设置体验与即时纠错

> **发布范围**：从 Preview 验证后纳入正式版的设置、纠错、图标与浮窗定位改进。本版不包含焦点策略实验，也不包含下一轮浮窗动效实验。
> **涉及文件**：assets/icon.ico、assets/tray.ico、assets/tray_16.png、src/config.py、src/gui.py、src/hotkey.py、src/main.py、src/pill.py、src/tray.py、src/logger.py、installer/yurun_setup.iss、README.md、RELEASE.md、CHANGELOG.md

### 改动点

- **设置窗口重设计**：使用原生 Windows 标题栏，移除仿 macOS 的三色装饰；扩大窗口与热键区，补充常用热键快捷选择与清晰的触发方式说明。
- **记忆设置窗口位置**：拖动后正常关闭或保存，下一次打开会出现在相同位置；显示器布局变化而导致位置不可见时，自动回到屏幕中央。
- **API Key 明确确认**：两个 Key 输入框都增加独立「确定」按钮，输入或更换后可即时保存；底部「保存并应用」继续负责热键及其它设置。
- **热键即时应用**：保存前真实验证系统热键注册；新热键无法使用时保留原热键，不要求重启。
- **纠错更顺手**：选中文字后使用 Ctrl + ` 打开纠错，点击「替换并存入词库」会立即替换该选区并保存词库，同时恢复原剪贴板内容；没有选区时仍可只保存词库。
- **图标与定位**：正式版换用新程序图标与专用 16×16 托盘图标；浮窗记录本轮输入开始时的目标窗口，避免偶发定位到屏幕左上角。

### 验证

- Python 编译检查、代码空白检查通过。
- 设置窗口位置保存/恢复逻辑具备越界回中保护；浮窗定位锚点只影响位置，不引入新的焦点策略。
- 正式打包前保留 v1.3.0 可执行文件作为本地回退点。

### 行为变化

- 设置不再每次回到屏幕中央；修改 Key 或主热键的结果会在当前操作中明确反馈。
- 对已选中的识别错误，纠正后的词会立即替换到原位置，无须用户再手动删改。
...
> **涉及文件**：src/main.py、src/tray.py、  src/refiner.py、src/logger.py、installer/yurun_setup.iss、README.md、CHANGELOG.md
> **背景**：v1.1.1 已解决浮窗打磨，但用户提出「大部分短句其实不需要 LLM 润色、只想去掉口水字并秒出」的需求。本版在系统托盘加入「润色」勾选开关，关闭后走纯本地「轻清洗」直出；同时修复了 v1.1.x 中两个会令 exe 双击无法启动的致命 bug。

### 改动点

- **① 托盘「润色」勾选开关**（`src/tray.py` + `src/main.py`）：
  - 系统托盘右键菜单新增 `润色` 勾选项（`pystray.MenuItem` + `checked=` 兼容写法，跨 pystray 版本通用），默认勾选（= 原 LLM 润色行为）。
  - `main.py` 新增 `_toggle_refine` 回调：翻转 `config.json` 的 `refine_enabled` 并持久化；`main._after_transcribe` 新增「总闸」——当 `refine_enabled=False` 时直接走轻清洗直出，不调 LLM、不显示「正在润色」中间态（轻清洗毫秒级，显示中间态反而晃眼）。
  - 状态持久化到 `%APPDATA%\Yurun\config.json`，重启后保持。

- **② 轻清洗规则（`src/refiner.py` 新增 `light_clean`）**——「润色关」模式的纯本地处理，毫秒级、不调大模型：
  - **删除**：仅 4 个低风险语气词 `呃 / 嗯 / 哦 / 额 / 唉`（口语叹息/停顿，删了不伤语义）。
  - **保留**（不删）：所有双字/多字填充词 `这个 / 那个 / 就是 / 其实 / 然后 / 的话 / 对吧 / 反正 / 就是说`（它们常作实义，一刀切会误伤「这个方案不错」「然后我们走」）；以及 `啊 / 呀 / 哎 / 吧`（保留「我的天啊」「你好呀」「哎呀」「算了吧」）。
  - **末尾标点规则**：短句（≤5 字）或纯数字串（如 `12345`/`3.14`，带单位如 `128元` 不算）剥「句末标点」（。．.!?；： 及英文对应，连续末尾全剥）；正常长句（>5 字且非纯数字）保留末尾标点。句末标点不含逗号/顿号/波浪号。
  - **明确不做**：重复词合并（避免「让我想想」→「让我想」）、通用停用词删除（的/是/了/我 等）。

- **③ 修复 exe 双击无法启动（两个致命 bug）**：
  - **pystray 兼容性**：原代码用 `pystray.CheckMenuItem`（老版本 pystray 无此类），托盘线程抛 `AttributeError` 崩溃。改为跨版本通用的 `pystray.MenuItem(..., checked=...)`。
  - **PyInstaller `--noconsole` 崩溃**：打包为无控制台窗口时 `sys.stdout/stderr` 为 `None`，logger 的 `StreamHandler` 首次写日志即抛 `AttributeError` 并连带中断启动（异常被吞、日志无痕）。改为仅在 `sys.stdout is not None` 时才挂控制台 handler。

- **④ 版本号**：`logger.py` `YURUN_VERSION` 与 `yurun_setup.iss` `MyAppVersion` 同步为 **1.2.1**（1.1.1 → 1.2.1）。

### 验证

- 规则单元测试（14+ 用例）：`今天你好呀`→`今天你好呀`、`这个方案不错`→原样、`12345。`→`12345`、`价格128元。`→保留句号、`哎呀`→保留、`呃嗯哦额唉`→空，全部符合预期。
- 打包验证：`dist/语润.exe` 经独立进程启动可正常常驻、托盘出现「润色」勾选项，无崩溃日志。
- 真机：用户实测「关润色 → 秒出、去口水字」体验符合预期。

### 行为变化

- 新增托盘「润色」开关：**开**（默认）= LLM 大改写；**关** = 轻清洗秒出。两种模式均可用，满足「多数短句免等待」诉求。
- 轻清洗不再误伤本义双字词与语气词；仅清理极低风险语气词与短句/纯数字句末标点。
- **已知风险（非 bug）**：流式识别中浮窗可能重复显示「有点意思」等，是火山 SAUC 中间结果抖动，松手后收敛为一次；本地无法还原，属正常 ASR 行为。

### 未合并/后续

- `pre/` 测试目录为本次开发的试验分支，验证通过后已并入主干，可删除。

---

## [1.1.1] — 2026-08-23 · Partial 浮 (浮窗打磨（钉左缘不晃 + 逐字吸走动画定速 30ms）`)
